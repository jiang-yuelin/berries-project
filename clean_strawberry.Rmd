...
### Total Data for each states across the years
```{r}
...
# Set unit in millions $
usd %<>% mutate(Value= ifelse(nchar(Value)<8, "000000000", Value))

usd$Value <- substr(usd$Value, 1, nchar(usd$Value)-8)
usd$Value <- as.numeric(sub(",", "", usd$Value, fixed = TRUE))
...
for (i in state_list) {
  ...
  plot(x =stateTotal$Year, y = stateTotal$Value, xlab="Year", ylab=paste(i),
       title(main = "Production in millions $"), pch =16, ylim =c(0,2900), xlim = c(2015, 2019))
}

```

### Relationship between use of fertilizers and yield per acre.
